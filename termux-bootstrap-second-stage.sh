...
export TERMUX_PACKAGE_ARCH="arm"
...
show_help() {
        ...
        cat <<'HELP_EOF'
termux-bootstrap-second-stage.sh runs the second stage of >
...
run_bootstrap_second_stage() {
        ...
        if ! ln -s "termux-bootstrap-second-stage.sh" \
                "/data/data/com.termux/files/usr/etc/termux/bootstrap/termux-bootstrap-second-stage.sh.lock" 2>/dev/null; then
...
run_package_postinst_maintainer_scripts() {
               ...
               # - https://github.com/guillemj/dpkg/blob/1.22.6/src/main/script.c#L107
                if [ -d "${TERMUX_PREFIX}/var/lib/dpkg/info" ]; then
                        ...
                        local dpkg_force_things="security-mac,downgrade"
                                                                                                                                              # - https://manpages.debian.org/testing/dpkg/dpkg.1.en.html#D                                                         # - https://manpages.debian.org/unstable/dpkg/dpkg.1.en.html#DPKG_DEBUG
                        ...
                        if [[ "$DPKG_DEBUG" =~ ^0[0-7]{1,6}$ ]] && [[ "$(( DPKG_DEBUG & dbg_scripts ))" != "0" ]]; th>
                                maintscript_debug=1
                        fi
   ...
   # - https://github.com/guillemj/dpkg/blob/1.22.6/src/main/script.c#L99-L130
                                        ...
                                        # - https://wiki.debian.org/Teams/Dpkg/Spec/InstallBootstrap#Detached_chroot_>
                                        ...
                                        # which may cause problems for packages which try                                                                     # to use Android rootfs paths instead of Termux                                                                       # rootfs paths.
                                        cd / || exit $?

                                        # Export internal environment variables that
                                        # `dpkg` exports for maintainer scripts.
                                        # - https://manpages.debian.org/testing/dpkg/dpkg.1.en.html#Internal_environm>
                ...
                if [ -d "${TERMUX_PREFIX}/var/lib/pacman/local" ]; then
                        ...
                        for script_path in "${TERMUX_PREFIX}/var/lib/pacman/local/"*/install; do
                                ...
                                local package_name_and_version_pkgver="${package_dir_basename%"$package_version_pkgre>
                                # Trim trailing dash "-"
                                ...
                                if [[ ! "$package_version" =~ ^([0-9]+:)?[^-]+-[^-]+$ ]]; then
                                        log_error "The package_version '$package_version' extracted from package_dir_>
                                        return 1
                                fi
                                ...
                                (
                                        ...
                                        # > Each function is run chrooted inside the pacman install directory. See th>
                                        ...
                                        # - https://gitlab.archlinux.org/pacman/pacman/-/blob/v6.1.0/src/pacman/conf.>
                                        # - https://gitlab.archlinux.org/pacman/pacman/-/blob/v6.1.0/lib/libalpm/alpm>
                                        # - https://gitlab.archlinux.org/pacman/pacman/-/blob/v6.1.0/lib/libalpm/alpm>
                                        # - https://gitlab.archlinux.org/pacman/pacman/-/blob/v6.1.0/lib/libalpm/util>
                                        # - https://man7.org/linux/man-pages/man2/chroot.2.html
                                        # But since Android apps cannot run chroot
                                        # without root access, chroot is disabled by                                                                          # Termux pacman package and only current working                                                                      # directory is changed to the Android rootfs `/`.
...
# Note that Termux rootfs is under private app
                                        ...
                                        # rootfs paths.
                                        # - https://github.com/termux/termux-packages/blob/953b9f2aac0dc94f3b99b2df6a>
                                        cd "/" || exit $?
                                        ...
                                        return_value=$?
                                        if [ $return_value -ne 0 ]; then
                                                log_error "Failed to source '$package_dir_basename' package install i>
                                                exit $return_value
                                        fi

                                        if [[ "$(type -t post_install 2>/dev/null)" == "function" ]]; then                                                            # cd again in case install file sourced changed the directory.                                                        cd "/" || exit $?
                                                ...
                                                return_value=$?
                                                if [ $return_value -ne 0 ]; then
                                                        log_error "Failed to run '$package_dir_basename' package post>
                                                        exit $return_value
                                                fi
                                        fi
                                ) || return $?
